...
export const LAYOUT_STORAGE_KEY = "svg.dashboard.layout.v4";
export const LAYOUT_STORAGE_KEY_V3 = "svg.dashboard.layout.v3";
export const LAYOUT_STORAGE_KEY_V2 = "svg.dashboard.layout.v2";
export const LAYOUT_STORAGE_KEY_V1 = "svg.dashboard.layout.v1";
/** Obsolete keys — read only to archive; never used for placement after v4. */
export const OBSOLETE_LAYOUT_KEYS = Object.freeze([
...
/**
 * Custom panel dragging is disabled during deployment recovery.
 * Opt in with window.SVG_CUSTOM_LAYOUT_ENABLED === true or setCustomLayoutEnabled(true).
 * @type {boolean}
 */
let customLayoutEnabled = typeof window !== "undefined"
    && window.SVG_CUSTOM_LAYOUT_ENABLED === true;

/**
 * @param {boolean} enabled
 */
export function setCustomLayoutEnabled(enabled) {
 ...
 * @returns {boolean}
 */
export function isCustomLayoutEnabled() {
    return customLayoutEnabled;
}
...
export const DEFAULT_LAYOUT = {
    // Kept for tests / optional custom-layout mode.
    // Visual layout prefers nested CSS (.dashboard-upper / .dashboard-right).
    // Voice Controls (~58%) | Chat (upper-right ~42%)
    //                       | Appearance (below Chat, bottoms align with Voice)
    // Voice Groups (full width)
    voice: { id: "voice", col: 1, row: 1, colSpan: 7, rowSpan: 8 },
    chat: { id: "chat", col: 8, row: 1, colSpan: 5, rowSpan: 5 },
    appearance: { id: "appearance", col: 8, row: 6, colSpan: 5, rowSpan: 3 },
    groups: { id: "groups", col: 1, row: 9, colSpan: 12, rowSpan: 4 }
};

const PANEL_IDS = Object.keys(DEFAULT_LAYOUT);
/** Required panels that must always be present after migration. */
export const REQUIRED_PANEL_IDS = Object.freeze(["voice", "chat", "appearance", "groups"]);

export const INTERACTIVE_SELECTOR =
    "button, a, input, textarea, select, option, [role=switch], [contenteditable=true], .no-drag";

/**
 * True when the event target is a real control that must not start a panel drag.
 * The panel handle itself may be keyboard-focusable but must still allow pointer drag.
 * @param {EventTarget|null} target
 * @param {Element|null} handle
 * @returns {boolean}
 */
export function isInteractiveDragTarget(target, handle) {
    if (!target || typeof target.closest !== "function") {
        return false;
    }
    // In browsers, prefer Element instances; Node tests may pass duck-typed targets.
    if (typeof Element !== "undefined" && !(target instanceof Element) && !handle) {
        return false;
    }
    ...
    // Buttons inside the visually-hidden a11y move strip must not start a drag.
    if (handle && interactive !== handle && typeof handle.contains === "function" && handle.contains(interactive)) {
        return true;
    }
    // The handle itself is never treated as an interactive blocker.
    if (handle && (interactive === handle || (typeof handle.contains === "function" && handle.contains(target)))) {
        return interactive !== handle && !!interactive.closest("button, a, input, textarea, select");
    }
    return true;
}
...
export function validateLayout(value) {
    ...
    if (version !== LAYOUT_VERSION || !raw.panels || typeof raw.panels !== "object") {
        return null;
    }
    ...
    // Insert any missing required panels (Appearance must never be omitted by a
    // stale saved layout that only knew voice/chat/groups).
...
/**
 * Remove obsolete layout keys so broken v1/v2/v3 placements cannot override v4 defaults.
 * @param {string} [storageKey]
 */
export function archiveObsoleteLayouts(storageKey = LAYOUT_STORAGE_KEY) {
...
export function loadLayout(storageKey = LAYOUT_STORAGE_KEY) {
    // Recovery mode: never apply saved placements; always use canonical defaults.
    if (!customLayoutEnabled) {
        if (storageKey === LAYOUT_STORAGE_KEY) {
            archiveObsoleteLayouts(storageKey);
        }
        return cloneLayout(DEFAULT_LAYOUT);
    }
    try {
        ...
        if (!raw && storageKey === LAYOUT_STORAGE_KEY) {
            const hadObsolete = OBSOLETE_LAYOUT_KEYS.some((key) => localStorage.getItem(key));
            if (hadObsolete) {
                archiveObsoleteLayouts(storageKey);
            }
            return cloneLayout(DEFAULT_LAYOUT);
...
/**
 * Clears current + obsolete layout keys (v1–v4).
 * @param {string} [storageKey]
 */
export function clearLayout(storageKey = LAYOUT_STORAGE_KEY) {
    ...
    if (storageKey === LAYOUT_STORAGE_KEY) {
        archiveObsoleteLayouts(storageKey);
        // Also drop any lingering v4 when resetting during recovery.
...
export function snapToGrid(gridRect, clientX, clientY, colSpan, rowSpan, gap = 16) {
    const resolvedGap = Number.isFinite(gap) && gap > 0 ? gap : 16;
    const innerW = Math.max(1, gridRect.width - resolvedGap * (GRID_COLS - 1));
    const innerH = Math.max(1, gridRect.height - resolvedGap * (GRID_ROWS - 1));
    const cellW = innerW / GRID_COLS;
    const cellH = innerH / GRID_ROWS;
    const x = clientX - gridRect.left;
    const y = clientY - gridRect.top;
    let col = Math.round(x / (cellW + resolvedGap)) + 1;
    let row = Math.round(y / (cellH + resolvedGap)) + 1;
...
/**
 * Resolve dashboard panel gap from computed CSS (falls back to 16px).
 * @param {Element|null|undefined} gridEl
 * @returns {number}
 */
export function readPanelGapPx(gridEl) {
    if (!gridEl || typeof getComputedStyle !== "function") {
        return 16;
    }
    const gap = parseFloat(getComputedStyle(gridEl).columnGap || getComputedStyle(gridEl).gap);
...
export class DashboardLayoutController {
     ...
     * @param {boolean} [options.customLayoutEnabled]
    ...
    constructor(options) {
        ...
        if (typeof options.customLayoutEnabled === "boolean") {
            setCustomLayoutEnabled(options.customLayoutEnabled);
        } else if (typeof window !== "undefined") {
            setCustomLayoutEnabled(window.SVG_CUSTOM_LAYOUT_ENABLED === true);
        }
        this.customLayoutEnabled = isCustomLayoutEnabled();
    ...
    init() {
        ...
        this.bound = true;
        // Drop obsolete v1–v3 keys so stale placements cannot override canonical CSS.
        archiveObsoleteLayouts(this.storageKey);
        ...
        if (!this.customLayoutEnabled) {
            this.#applyCanonicalOnly();
            this.mq.addEventListener?.("change", () => this.#applyCanonicalOnly());
            window.addEventListener("resize", () => this.#applyCanonicalOnly());
            return;
        }

        this.gridEl.classList.add("is-custom-layout-enabled");
        this.#ensureDropHint();
        ...
        for (const [id, el] of Object.entries(this.panels)) {
            ...
            if (handle) {
                ...
                // Do NOT set role="button" on the whole handle — that previously matched
                // INTERACTIVE_SELECTOR and blocked every pointer drag start.
    ...
    /**
     * Canonical nested CSS layout — no grid placement, no pointer listeners, no localStorage.
     */
    #applyCanonicalOnly() {
        this.layout = cloneLayout(DEFAULT_LAYOUT);
        this.gridEl.classList.add("is-canonical-layout");
        this.gridEl.classList.remove("is-custom-layout-enabled", "is-desktop-grid");
        ...
        for (const id of PANEL_IDS) {
            ...
            el.classList.remove("is-dragging", "is-drag-preview", "panel-snap");
            if (mobile) {
                const order = id === "voice" ? 1
                    : id === "chat" ? 2
                        : id === "appearance" ? 3
                            : 4;
    ...
    apply(layout, opts = {}) {
        ...
        for (const id of PANEL_IDS) {
            ...
            if (mobile) {
                el.style.removeProperty("grid-column");
                el.style.removeProperty("grid-row");
                // Mobile stack: Voice → Chat → Appearance → Groups.
                const order = id === "voice" ? 1
                    : id === "chat" ? 2
                        : id === "appearance" ? 3
                            : 4;
    ...
    #onPointerDown(event, panelId) {
        ...
        if (isInteractiveDragTarget(target, handle)) {
            return;
        }
        ...
        event.preventDefault();
        // Capture on the handle that received pointerdown so move/up keep delivering.
        try {
    ...
    #onPointerUp(event) {
        ...
        try {
            const captureEl = this.drag.captureEl || panel;
        ...
        const snapped = snapToGrid(rect, cx, cy, place.colSpan, place.rowSpan, readPanelGapPx(this.gridEl));
    ...
    #cancelDrag(restoreLayout) {
        ...
        try {
            const captureEl = this.drag.captureEl || panel;
            captureEl?.releasePointerCapture?.(this.drag.pointerId);
        } catch {
            // ignore
        }
        this.drag = null;
