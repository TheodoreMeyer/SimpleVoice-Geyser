...
export class SvgAudio {
    ...
    async startMic(deviceId) {
        ...
        // Capture-only worklet: one input, one muted output keep-alive (some engines
        // pause worklets with no path toward the destination).
    ...
    #handleMicMessage(event, generation) {
        ...
        // AudioContext.sampleRate is authoritative for MediaStreamSource timing.
        // Worklet sampleRate should match; only adopt it when it is a plausible
        // hardware rate and does not invent a lower rate that would upsample.
        let declaredRate = this.contextSampleRate;
        if (Number.isFinite(workletRate) && workletRate > 0) {
            if (Math.abs(workletRate - this.contextSampleRate) > 0.5) {
                Logger.debug(
                    `[Audio] workletRate=${workletRate} differs from contextRate=${this.contextSampleRate}; ` +
                    `preferring contextRate for resampling`
                );
            }
            // Never treat a lower worklet rate as authoritative when it would
            // upsample (that produces ~2× frames when capture is actually faster).
            if (workletRate >= this.contextSampleRate * 0.98) {
        ...
        // Keep using the calibrated effective rate when wall-clock detection has
        // already raised inputRate above the declared context rate.
        const inputRate = this.effectiveInputRate >= declaredRate
            ? this.effectiveInputRate
            : declaredRate;
        this.#ensureResampler(inputRate);
        ...
        this.#maybeCalibrateInputRate(now);
    ...
    #maybeReportCaptureMetrics(now) {
        ...
        if (!snap || snap.wallIntervalMs < MIN_CALIBRATION_WALL_MS / 2) {
            return;
        }
        this.captureMetrics.lastReportAt = now;
        Logger.debug(formatCaptureMetrics(snap));
    ...
    /**
     * Wall-clock calibration: if source samples arrive much faster than the
     * declared AudioContext rate, the identity resampler would emit ~100 fps.
     * Raise effectiveInputRate so StreamingResampler downsamples to 48 kHz.
     * @param {number} now
     */
    #maybeCalibrateInputRate(now) {
        const snap = this.captureMetrics.snapshot(now);
        const calibrated = this.captureMetrics.suggestCalibratedInputRate(snap);
        if (calibrated == null) {
            return;
        }
        if (Math.abs(calibrated - this.effectiveInputRate) < 1) {
            return;
        }
        Logger.debug(
            `[Audio] wall-clock rate calibration measured=${snap.measuredInputRate.toFixed(1)} ` +
            `declared=${snap.declaredInputRate} → effective=${calibrated}`
        );
        this.effectiveInputRate = calibrated;
        this.#ensureResampler(calibrated);
        this.captureMetrics.calibrated = true;
        this.#syncRuntimeCounts();
        this.captureMetrics.rollWindow(now);
    }
