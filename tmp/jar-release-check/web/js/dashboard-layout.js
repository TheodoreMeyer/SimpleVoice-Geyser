...
export const LAYOUT_STORAGE_KEY = "svg.dashboard.layout.v3";
...
export const DEFAULT_LAYOUT = {
    // Desktop:
    // Voice Controls (left) | Chat (upper-right)
    //                       | Appearance (below Chat, bottoms align with Voice)
    // Voice Groups (full width)
    voice: { id: "voice", col: 1, row: 1, colSpan: 5, rowSpan: 8 },
    chat: { id: "chat", col: 6, row: 1, colSpan: 7, rowSpan: 5 },
    appearance: { id: "appearance", col: 6, row: 6, colSpan: 7, rowSpan: 3 },
...
export function validateLayout(value) {
    ...
    // Accept v1/v2/v3 payloads; migrate missing required panels (esp. appearance).
    if (![1, 2, LAYOUT_VERSION].includes(version) || !raw.panels || typeof raw.panels !== "object") {
        return null;
    }
    ...
    // Migrate v2 chat that occupied the full right column: shrink chat and
    // place appearance beneath it so bottoms still align with voice.
    if (!raw.panels.appearance && panels.chat && panels.voice) {
        const chat = panels.chat;
        const voice = panels.voice;
        if (chat.row === voice.row && chat.rowSpan === voice.rowSpan && chat.rowSpan >= 6) {
            const appearanceSpan = Math.max(3, Math.floor(chat.rowSpan / 3));
            const chatSpan = chat.rowSpan - appearanceSpan;
            if (chatSpan >= 3) {
                panels.chat = { ...chat, rowSpan: chatSpan };
                panels.appearance = {
                    id: "appearance",
                    col: chat.col,
                    row: chat.row + chatSpan,
                    colSpan: chat.colSpan,
                    rowSpan: appearanceSpan
                };
            }
        }
    }
...
export function loadLayout(storageKey = LAYOUT_STORAGE_KEY) {
    try {
        ...
        if (!raw && storageKey === LAYOUT_STORAGE_KEY) {
            raw = localStorage.getItem(LAYOUT_STORAGE_KEY_V2)
                || localStorage.getItem(LAYOUT_STORAGE_KEY_V1);
            if (raw) {
