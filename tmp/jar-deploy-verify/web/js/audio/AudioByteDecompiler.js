...
import {loadOpusDecoderModule} from "./vendor/opus-decoder-loader.js";
...
async function initWasmDecoder() {
    ...
    try {
        const mod = await loadOpusDecoderModule();
        ...
        wasmReady = true;
        Logger.log("[AudioRX] Opus WASM decoder ready (local).");
    } catch (err) {
        wasmInitError = err?.message || String(err);
        Logger.log(`[AudioRX] Opus WASM decoder unavailable: ${wasmInitError}`);
        try {
            if (typeof document !== "undefined") {
                const el = document.querySelector("[data-svg='dash.mic-error']")
                    || document.querySelector("[data-svg='dash.decoder-error']");
                if (el) {
                    el.hidden = false;
                    el.textContent = "Audio playback decoder failed to load. Chat and groups still work.";
                }
            }
        } catch {
            // UI may not be mounted yet — non-fatal.
        }
    }
}
