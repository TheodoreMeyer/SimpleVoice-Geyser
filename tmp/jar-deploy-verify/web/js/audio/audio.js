...
/** @type {WeakSet<object>} */
const LIVE_AUDIO_PIPELINES = new WeakSet();
...
export class SvgAudio {
    ...
    static METRICS_EVERY_MS = 5000;
    static AUDIO_SETTINGS_KEY = "svg.audio.settings.v1";
    static IDLE_GAP_MS = 100;
    ...
    constructor() {
        ...
        this.captureMetrics = new CaptureProductionMetrics();
        this.effectiveInputRate = TARGET_SAMPLE_RATE;
        this.workletMessageHandlerCount = 0;
        this.pipelineRegistered = false;
        this.rateGuardRebuildInFlight = false;
        this.lastBinarySendAt = 0;
        this.creationStack = "";
        this.lastQuantumSequence = 0;
        this.duplicateQuantumDrops = 0;
        this.workletSourceSamplesProcessed = 0;
        this.workletOutputSamplesProduced = 0;
        this.workletFramesProduced = 0;
        this.activeBinarySenderCount = 0;
        this.lastSenderInvariantLogAt = 0;
    ...
    resolveAudioModuleUrl(moduleName) {
        ...
        if (buildId && buildId !== "0.1.3-c471505-dirty-20260810T080751Z" && !String(buildId).includes("@@")) {
    ...
    /**
     * @returns {CaptureProductionMetrics}
     */
    getCaptureMetrics() {
    ...
    async startMic(deviceId) {
        ...
        this.outgoingFrames = 0;
        this.outgoingBytes = 0;
        this.captureMetrics.failSafeTripped = false;
        ...
        this.creationStack = (new Error("mic-pipeline-create")).stack || "";
        Logger.debug(
            ...
            `agc=${settings.autoGainControl} activePipelines=${livePipelineCount + 1}`
        );

        this.micSource = this.audioContext.createMediaStreamSource(this.microphoneStream);
        // Capture-only worklet matching v0.1.3: source → worklet only.
        // Do NOT connect mic output to destination — that keep-alive path was
        // observed to contribute to duplicated quantum delivery (~100 fps at 48 kHz).
    ...
    #registerLivePipeline() {
        if (!this.pipelineRegistered) {
            LIVE_AUDIO_PIPELINES.add(this);
            livePipelineCount++;
            this.pipelineRegistered = true;
        }
        if (livePipelineCount > 1) {
            Logger.debug(
                `[Audio] INVARIANT violated activeBrowserPipelines=${livePipelineCount} ` +
                `retiring stale graphs stack=${this.creationStack}`
            );
        }
    }

    #unregisterLivePipeline() {
        if (this.pipelineRegistered) {
            LIVE_AUDIO_PIPELINES.delete(this);
            livePipelineCount = Math.max(0, livePipelineCount - 1);
            this.pipelineRegistered = false;
        }
    }

    #syncRuntimeCounts() {
        const tracks = this.microphoneStream ? this.microphoneStream.getAudioTracks() : [];
        const liveTracks = tracks.filter((t) => t && t.readyState === "live");
        this.captureMetrics.setRuntimeCounts({
            activeMediaStreams: this.microphoneStream ? 1 : 0,
            activeTracks: tracks.length,
            activeLiveTracks: liveTracks.length,
            activeAudioContexts: this.audioContext ? 1 : 0,
            activeWorkletNodes: this.micNode ? 1 : 0,
            activeWorkletPorts: this.micNode?.port ? 1 : 0,
            activeWorkletMessageHandlers: this.workletMessageHandlerCount,
            activeBrowserPipelines: livePipelineCount,
            actualContextSampleRate: this.contextSampleRate,
            actualTrackSampleRate: this.appliedTrackSettings?.sampleRate || 0,
            actualChannelCount: this.contextChannelCount,
            declaredInputRate: this.contextSampleRate,
            effectiveInputRate: this.effectiveInputRate
        });
    }
    ...
    #handleMicMessage(event, generation) {
        ...
        const {
            samples,
            speech,
            sampleRate: workletRate,
            channels,
            energy,
            quantumSequence,
            sourceSamplesProcessed,
        ...
        // Drop duplicate quantum deliveries (same worklet sequence seen twice).
        if (Number.isFinite(quantumSequence) && quantumSequence > 0) {
            if (quantumSequence <= this.lastQuantumSequence) {
                this.duplicateQuantumDrops++;
                if (this.duplicateQuantumDrops === 1 || (this.duplicateQuantumDrops % 50) === 0) {
                    Logger.debug(
                        `[Audio] dropping duplicate quantum seq=${quantumSequence} ` +
                        `last=${this.lastQuantumSequence} drops=${this.duplicateQuantumDrops}`
                    );
                }
                return;
            }
            this.lastQuantumSequence = quantumSequence;
        }
        if (Number.isFinite(sourceSamplesProcessed)) {
            this.workletSourceSamplesProcessed = sourceSamplesProcessed;
        }
        if (Number.isFinite(outputSamplesProduced)) {
            this.workletOutputSamplesProduced = outputSamplesProduced;
        }
        if (Number.isFinite(framesProduced)) {
            this.workletFramesProduced = framesProduced;
        }

        // AudioContext.sampleRate is authoritative. Never reinterpret from wall-clock.
        let declaredRate = this.contextSampleRate;
        if (Number.isFinite(workletRate) && workletRate > 0
            && Math.abs(workletRate - this.contextSampleRate) <= 0.5) {
            declaredRate = workletRate;
        }
        this.effectiveInputRate = declaredRate;
        this.#ensureResampler(declaredRate);
        ...
        // Worklet already downmixes planar channels to mono of length frameLength.
        const mono = samples;
        ...
        for (const frame of frames) {
            ...
            if (this.shouldSendPacket(mode, packetHasSpeech, pttActive)) {
                // Re-check generation immediately before handing off to the sender.
                if (generation !== this.activeMicGeneration) {
                    return;
                }
                ...
                this.outgoingBytes += pcm.byteLength;
                binarySent++;
                this.lastBinarySendAt = now;
                // Detach buffer so callers cannot observe a recycled view.
                this.micHandler?.(pcm.buffer.slice(0), generation);
            }
        }

        // Metrics only accumulate during active capture callbacks (not idle gaps).
        this.#syncRuntimeCounts();
        this.captureMetrics.recordActive(
            now,
            mono.length,
            mono.length,
            resampled.length,
        ...
        this.#maybeReportCaptureMetrics(now);
        ...
        if (now - this.lastDiagAt >= SvgAudio.DIAG_EVERY_MS) {
            ...
            Logger.debug(
                `[Audio] diag pipeline=${this.pipelineId} inRate=${this.effectiveInputRate} ` +
                `declared=${declaredRate} channels=${this.contextChannelCount} ratio=${ratio.toFixed(6)} ` +
                `frames=${this.outgoingFrames} bytes/frame=${FRAME_BYTES} ~fps=${fps.toFixed(1)} ` +
                `pending=${this.frameAccumulator.pendingSamples()} pipelines=${livePipelineCount} ` +
                `workletSource=${this.workletSourceSamplesProcessed} ` +
                `dupQuantumDrops=${this.duplicateQuantumDrops}`
            );
            this.outgoingFrames = 0;
            this.outgoingBytes = 0;
        }
     ...
     * @param {number} now
     */
    #maybeReportCaptureMetrics(now) {
        if (now - this.captureMetrics.lastReportAt < SvgAudio.METRICS_EVERY_MS) {
            return;
        }
        const snap = this.captureMetrics.snapshot(now);
        if (!snap || snap.wallIntervalMs < MIN_METRICS_WALL_MS / 2) {
            return;
        }
        this.captureMetrics.lastReportAt = now;
        Logger.debug(
            `${formatCaptureMetrics(snap)} workletSource=${this.workletSourceSamplesProcessed}` +
            ` workletOut=${this.workletOutputSamplesProduced}` +
            ` workletFrames=${this.workletFramesProduced}` +
            ` dupQuantumDrops=${this.duplicateQuantumDrops}`
        );

        if (snap.activeMediaStreams > 1
            || snap.activeLiveTracks > 1
            || snap.activeAudioContexts > 1
            || snap.activeWorkletNodes > 1
            || snap.activeWorkletMessageHandlers > 1
            || snap.activeBrowserPipelines > 1) {
            Logger.debug(
                `[Audio] INVARIANT duplicate capture detected streams=${snap.activeMediaStreams} ` +
                `liveTracks=${snap.activeLiveTracks} ctx=${snap.activeAudioContexts} ` +
                `worklets=${snap.activeWorkletNodes} handlers=${snap.activeWorkletMessageHandlers} ` +
                `pipelines=${snap.activeBrowserPipelines} stack=${this.creationStack}`
            );
            void this.#failSafeRebuild("duplicate_pipeline");
            return;
        }

        // Overproduction after removing destination keep-alive / calibration is a real
        // duplicate-path bug — rebuild once, then surface a UI error.
        if (this.captureMetrics.noteOverproduction(snap)) {
            Logger.debug(
                `[Audio] overproduction fail-safe represented/wall=${snap.representedAudioDurationPerWall.toFixed(3)} ` +
                `fps=${snap.pcmFramesPerSecond.toFixed(1)} — controlled rebuild`
            );
            void this.#failSafeRebuild("overproduction");
            return;
        }
        if (this.captureMetrics.failSafeTripped) {
            Logger.debug("[Audio] overproduction persists after rebuild — pipeline error");
            this.onPipelineError?.(
                "Audio pipeline is producing frames faster than real time. Check microphone settings."
            );
            this.stopMic();
        }

        this.captureMetrics.rollWindow(now);
    }

    /**
     * @param {string} reason
     */
    async #failSafeRebuild(reason) {
        if (this.rateGuardRebuildInFlight || this.micRebuildInFlight) {
            return;
        }
        this.rateGuardRebuildInFlight = true;
        try {
            Logger.debug(`[Audio] fail-safe rebuild reason=${reason} gen=${this.activeMicGeneration}`);
            this.stopMic();
            if (this.audioRuntime.canCaptureMic) {
                await this.startMic(this.selectedDeviceId || undefined);
            }
        } catch (err) {
            Logger.debug(`[Audio] fail-safe rebuild failed: ${err?.message || err}`);
            this.onPipelineError?.("Audio pipeline failed to recover. Reload the page.");
        } finally {
            this.rateGuardRebuildInFlight = false;
        }
    }
    ...
    stopMic() {
        ...
        this.lastLevel = 0;
        this.effectiveInputRate = this.contextSampleRate || TARGET_SAMPLE_RATE;
