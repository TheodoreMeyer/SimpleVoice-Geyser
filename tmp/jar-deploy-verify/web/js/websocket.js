...
export class SvgWebSocket {
    ...
    constructor(audioController) {
        ...
        this.buildIdentityHandler = null;
        this.serverBuildId = null;
        this.protocolVersion = null;
        this.frontendSchema = null;
        this.#resetState();
    }
     ...
     *   frontendBuildIdExpected: string|null,
     *   protocolVersion: number|null,
     *   frontendSchema: number|null
     * }) => void} handler
     */
    onBuildIdentity(handler) {
        this.buildIdentityHandler = handler;
    }
    ...
    getBuildIdentity() {
        return {
            ...
            frontendSchema: this.frontendSchema
        };
    }
        ...
        this.ws.onopen = () => {
            ...
            this.ws.send(JSON.stringify({
                ...
                clientType: {
                    ...
                    protocolVersion: Number.isFinite(window.PROTOCOL_VERSION)
                        ? Number(window.PROTOCOL_VERSION)
                        : 3,
                    frontendSchema: Number.isFinite(window.FRONTEND_SCHEMA)
                ...
                try {
                    ...
                    if (isReadyPacket && !this.hasJoined) {
                        ...
                        if (isStructuredReady) {
                            this.serverBuildId = data.serverBuildId != null
                                ? String(data.serverBuildId)
                                : (data.frontendBuildIdExpected != null
                                    ? String(data.frontendBuildIdExpected)
                                    : null);
                            this.protocolVersion = data.protocolVersion != null
                                ? Number(data.protocolVersion)
                                : null;
                            this.frontendSchema = data.frontendSchema != null
                                ? Number(data.frontendSchema)
                                : null;
                            this.buildIdentityHandler?.({
                                serverBuildId: this.serverBuildId,
                                frontendBuildIdExpected: data.frontendBuildIdExpected != null
                                    ? String(data.frontendBuildIdExpected)
                                    : this.serverBuildId,
                                protocolVersion: this.protocolVersion,
                                frontendSchema: this.frontendSchema,
                                customLayoutEnabled: Object.prototype.hasOwnProperty.call(data, "customLayoutEnabled")
                                    ? data.customLayoutEnabled === true
                                    : false
                            });
                        }
