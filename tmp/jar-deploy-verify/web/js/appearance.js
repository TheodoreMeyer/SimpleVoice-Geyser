 ...
 * Independent of DashboardLayoutController.
 */

export const APPEARANCE_STORAGE_KEY = "svg.dashboard.appearance.v4";
export const APPEARANCE_STORAGE_KEY_V3 = "svg.dashboard.appearance.v3";
export const APPEARANCE_STORAGE_KEY_V2 = "svg.dashboard.appearance.v2";
export const APPEARANCE_STORAGE_KEY_V1 = "svg.dashboard.appearance.v1";
/** Obsolete keys — migrated then deleted. */
export const OBSOLETE_APPEARANCE_KEYS = Object.freeze([
...
/**
 * Migrates a v1 `{version:1, theme}` record to v4 `{accent, border}`.
 ...
 * @returns {AppearancePrefs}
 */
function migrateV2orV3(parsed) {
    const raw = /** @type {any} */ (parsed);
    return {
        accent: validateAccent(raw?.accent) || "default",
        border: validateBorder(raw?.border) || "default"
    };
}

/**
 * @param {unknown} parsed
 * @returns {AppearancePrefs|null}
 */
function parseAppearanceRecord(parsed) {
    ...
    if (
        ...
        || version === 3
        || (!Number.isFinite(version) && ("accent" in raw || "border" in raw))
    ) {
        return {
            accent: validateAccent(raw.accent) || "default",
            border: validateBorder(raw.border) || "default"
 ...
 */
export function archiveObsoleteAppearance(storageKey = APPEARANCE_STORAGE_KEY) {
    if (storageKey !== APPEARANCE_STORAGE_KEY) {
        return;
    }
    for (const key of OBSOLETE_APPEARANCE_KEYS) {
...
export function loadAppearance(storageKey = APPEARANCE_STORAGE_KEY) {
    try {
        ...
        if (!raw && storageKey === APPEARANCE_STORAGE_KEY) {
            raw = localStorage.getItem(APPEARANCE_STORAGE_KEY_V3)
                || localStorage.getItem(APPEARANCE_STORAGE_KEY_V2)
                || localStorage.getItem(APPEARANCE_STORAGE_KEY_V1);
            if (raw) {
                const parsed = JSON.parse(raw);
                const migrated = parseAppearanceRecord(parsed) || { accent: "default", border: "default" };
                saveAppearance(migrated.accent, migrated.border, APPEARANCE_STORAGE_KEY);
                archiveObsoleteAppearance(APPEARANCE_STORAGE_KEY);
                return migrated;
            }
        }
        if (!raw) {
        ...
        const parsed = JSON.parse(raw);
        const prefs = parseAppearanceRecord(parsed) || { accent: "default", border: "default" };
        // Upgrade a v3 payload that somehow landed under the v4 key.
        if (storageKey === APPEARANCE_STORAGE_KEY) {
            const version = Number(/** @type {any} */ (parsed)?.version);
            if (version !== APPEARANCE_VERSION) {
                saveAppearance(prefs.accent, prefs.border, APPEARANCE_STORAGE_KEY);
                archiveObsoleteAppearance(APPEARANCE_STORAGE_KEY);
            }
        }
        return prefs;
...
export function clearAppearance(storageKey = APPEARANCE_STORAGE_KEY) {
    ...
    if (storageKey === APPEARANCE_STORAGE_KEY) {
        archiveObsoleteAppearance(storageKey);
    }
}

/**
 * Apply accent + border appearance to the dashboard root only — never the
 * login view. Must work without a layout controller.
 */
