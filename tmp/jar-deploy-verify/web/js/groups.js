...
export class GroupsController {
    ...
    applySnapshot(data, options = {}) {
        ...
        const directoryRevision = Number(data.revision);
        const incomingMembershipRevision = Number(data.membershipRevision);
        const list = Array.isArray(data.groups) ? data.groups : [];
        const hasAuthoritativeMembership = Object.prototype.hasOwnProperty.call(data, "currentGroupId");
        const snapshotCurrentId = hasAuthoritativeMembership
            ? (data.currentGroupId == null || data.currentGroupId === ""
                ? null
                : String(data.currentGroupId))
            : undefined;

        if (Number.isFinite(directoryRevision) && directoryRevision < this.directoryRevision) {
            console.debug(
                `groups: snapshot revision=${directoryRevision} ignored (stale; local=${this.directoryRevision})`
            );
            return false;
        }

        const membershipIsNewer = Number.isFinite(incomingMembershipRevision)
            && incomingMembershipRevision > this.membershipRevision;

        if (hasAuthoritativeMembership) {
            if (membershipIsNewer || !Number.isFinite(incomingMembershipRevision)) {
                this.currentGroupId = snapshotCurrentId ?? null;
            } else if (snapshotCurrentId != null) {
                this.currentGroupId = snapshotCurrentId;
            }
            // Stale directory-only snapshot: keep local membership unless snapshot confirms join.
        } else {
            for (const g of list) {
                if (g?.joined) {
                    this.currentGroupId = String(g.uuid || "");
                    break;
                }
            }
        }

        const membershipIdentity = `${this.currentGroupId || "none"}|${list.length}|${list.map((g) =>
            `${g?.uuid}:${g?.memberCount || 0}:${this.currentGroupId && g?.uuid === this.currentGroupId ? 1 : 0}`
        ).join(",")}`;

        if (Number.isFinite(directoryRevision)) {
            if (
        ...
        if (Number.isFinite(incomingMembershipRevision)) {
            this.membershipRevision = Math.max(this.membershipRevision, incomingMembershipRevision);
        }
        this.lastMembershipIdentity = membershipIdentity;

        const preserveJoinedId =
            this.currentGroupId
            && !list.some((g) => g?.uuid === this.currentGroupId)
        ...
        for (const g of list) {
            if (g?.uuid) {
                ...
                normalized.joined = this.currentGroupId != null && normalized.uuid === this.currentGroupId;
        ...
        console.debug(
            `groups:reconcile directoryRevision=${this.directoryRevision} `
            + `membershipRevision=${this.membershipRevision} groups=${this.groups.size} `
            + `current=${this.currentGroupId || "none"}`
        );
        this.#updateCurrentGroup();
        this.#render();
        console.debug(`groups:render directoryRevision=${this.directoryRevision} groups=${this.groups.size}`);
        this.onSnapshot?.(this.directoryRevision, this.groups.size);
        return true;
    }
    ...
    #handleServerMessage(data) {
        ...
        if (Number.isFinite(revision) && revision < this.directoryRevision) {
            return;
        }
        if (Number.isFinite(revision) && revision > this.directoryRevision) {
        ...
        if (type === "membership_changed" || type === "group_joined") {
            ...
            const membershipRevision = Number(data.membershipRevision);
            ...
            if (!playerId || playerId === "self" || data.you === true || data.self === true) {
                if (joined && groupId) {
                    this.#applyJoinedFromResult(
                        groupId,
                        revision,
                        Number.isFinite(membershipRevision) ? membershipRevision : undefined
                    );
                    this.#render();
                } else if (!joined) {
                    this.#applyLeftFromResult({
                        previousGroupId: groupId || this.currentGroupId,
                        membershipRevision: Number.isFinite(membershipRevision) ? membershipRevision : undefined
                    });
                    this.#render();
                }
            }
    ...
    #handleOperationResult(result) {
        ...
        if (Number.isFinite(result?.revision) && result.revision > this.directoryRevision) {
            this.directoryRevision = result.revision;
        }
        if (Number.isFinite(result?.membershipRevision) && result.membershipRevision > this.membershipRevision) {
            this.membershipRevision = result.membershipRevision;
        }
        ...
        if (kind === "create") {
            const createJoined = result?.success === true && result.joined === true;
            const createFailed = !result?.success;
            if (createJoined || createFailed) {
                this.createPending = false;
                this.#setCreateFormBusy(false);
                this.createPasswordInput.value = "";
            }
        ...
        if (result?.success) {
            ...
            if (kind === "create") {
                ...
                if (result.joined === true && result.groupId) {
                    ...
                    this.#applyJoinedFromResult(
                        result.groupId || result.currentGroupId,
                        result.revision,
    ...
    #applyJoinedFromResult(groupId, revision, membershipRevision) {
        ...
        if (Number.isFinite(membershipRevision) && membershipRevision > this.membershipRevision) {
    ...
    #updateCurrentGroup() {
        ...
        const nextId = this.currentGroupId;
        const current = nextId ? this.groups.get(nextId) : null;
        this.currentGroupEl.textContent = current ? current.name : "None";
        this.leaveBtn.disabled = !this.enabled || !nextId;
        if (this.leaveBtn.dataset.defaultLabel) {
            this.leaveBtn.textContent = this.leaveBtn.dataset.defaultLabel;
        }
        if (nextId) {
        ...
        console.debug(
            `groups: membership group=${nextId || "none"} `
            + `directoryRevision=${this.directoryRevision} membershipRevision=${this.membershipRevision} `
            + `state=${this.membershipState}`
        );
    }
    ...
    #buildRow(group) {
        ...
        row.className = "group-card group-row" + (this.currentGroupId === group.uuid ? " is-joined" : "");
        ...
        if (this.currentGroupId === group.uuid) {
        ...
        joinBtn.textContent = (this.currentGroupId === group.uuid) ? "Joined" : "Join";
        joinBtn.disabled = !this.enabled || this.currentGroupId === group.uuid || this.pendingOps.size > 0;
