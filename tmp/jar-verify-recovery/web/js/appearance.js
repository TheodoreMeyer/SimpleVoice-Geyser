...
export const APPEARANCE_STORAGE_KEY = "svg.dashboard.appearance.v3";
...
/**
 * Migrates a v1 `{version:1, theme}` record to v3 `{accent, border}`.
...
function parseAppearanceRecord(parsed) {
    ...
    if (version === APPEARANCE_VERSION || (!Number.isFinite(version) && ("accent" in raw || "border" in raw))) {
        return {
            accent: validateAccent(raw.accent) || "default",
            border: validateBorder(raw.border) || "default"
        };
...
export function archiveObsoleteAppearance(storageKey = APPEARANCE_STORAGE_KEY) {
    ...
    localStorage.removeItem(APPEARANCE_STORAGE_KEY_V2);
...
export function loadAppearance(storageKey = APPEARANCE_STORAGE_KEY) {
    try {
        ...
        if (!raw && storageKey === APPEARANCE_STORAGE_KEY) {
            raw = localStorage.getItem(APPEARANCE_STORAGE_KEY_V2)
        ...
        const parsed = JSON.parse(raw);
        return parseAppearanceRecord(parsed) || { accent: "default", border: "default" };
    } catch {
        return { accent: "default", border: "default" };
    }
}
